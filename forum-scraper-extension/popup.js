...
const parseTopicsFromHtml = (text, forumId) => {
...
const scrapeOneForum = async (forum, seenIds) => {
  ...
  while (nextUrl) {
    try {
      ...
      const items = parseTopicsFromHtml(html, forum.id);
...
const runScrape = async () => {
  ...
  try {
    const forums = (await getForumList()).filter((f) => !f.sub_forum);
    if (forums.length === 0) {
      statsEl.textContent = 'No non-sub-forum entries found in forum.json.';
      log('No non-sub-forum forums to scrape.');
    } else {
